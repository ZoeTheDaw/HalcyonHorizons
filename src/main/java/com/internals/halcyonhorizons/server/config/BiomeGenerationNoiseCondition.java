...
public class BiomeGenerationNoiseCondition {
    ...
    private BiomeGenerationNoiseCondition(boolean disabledCompletely, int distanceFromSpawn, int horizonsRarityOffset, float[] continentalness, float[] erosion, float[] humidity, float[] temperature, float[] weirdness, float[] depth, String[] dimensions) {
        ...
        this.depth = depth;
        this.horizonsRarityOffset = horizonsRarityOffset;
    ...
    public boolean test(EventReplaceBiome event, VoronoiGenerator.VoronoiInfo info) {
        ...
        Vec3 rareBiomeCenter = HorizonsBiomeRarity.getRareBiomeCenter(info);
        ...
        return event.getWorldDimension() == null || dimensions.contains(event.getWorldDimension().location().toString());
    }
    ...
    public boolean test(int x, int y, int z, float unquantizedDepth, Climate.Sampler climateSampler, ResourceKey<Level> dimension, VoronoiGenerator.VoronoiInfo info) {
        ...
        return dimension == null || dimensions.contains(dimension.location().toString());
    }
    ...
    public static final class Builder {
        ...
        public Builder horizonsRarityOffset(int rarityOffset) {
