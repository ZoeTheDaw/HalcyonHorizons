...
public class CroliveTreeWithBranchesFeature extends CroliveTreeFeature {

    public CroliveTreeWithBranchesFeature(Codec<NoneFeatureConfiguration> codec) {
        super(codec);
    }
            ...
            for (int i = 0; i < 5 + randomSource.nextInt(10); i++) {
                ...
                if (level.getBlockState(branchPos).isCollisionShapeFullBlock(level, branchPos)) {
                    ...
                    if (level.getBlockState(branchPos).canBeReplaced()) {
                        level.setBlock(branchPos, HorizonsBlockRegistry.CROLIVE_BRANCH.get().defaultBlockState(), 3);
                    }
                }
            }
            return true;
